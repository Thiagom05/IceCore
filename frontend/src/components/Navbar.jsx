...
export default function Navbar() {
    ...
    return (
        <nav className="bg-bg-inverse text-white shadow-lg sticky top-0 z-50 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16 items-center">
                    ...
                    <Link to="/" className="flex items-center space-x-2 font-bold text-2xl tracking-tighter hover:opacity-90 transition">
                        <img src={logo} alt="Logo Pura Vida" className="h-9 w-9 rounded-full object-cover border-1 border-white/30" />
                        <span>Pura Vida</span>
                    </Link>
            ...
            {/* Mobile Menu Dropdown - slide-down suave */}
            <div
                className={`md:hidden bg-bg-inverse border-white/10 shadow-inner overflow-hidden transition-all duration-500 ease-in-out ${isOpen
                    ? 'max-h-64 opacity-100 border-t pb-4 pt-2 px-4'
